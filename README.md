...
ノーマライザーでのフィルター機能は、トークナイザーでフィルターするのに比べ処理負荷が大きくなりますが、検索時とスニペット時で検索ワードが異なることがなく、おおよそ適切なスニペットを行うことができます。  
ただし、スニペットの前の文字列がフィルターされた場合、フィルターされた文字列がスニペットに含まれてしまいます。  
たとえば、「実践!!C++入門」という文章に対して、記号をフィルターし、「C」という検索クエリで検索すると、「``<span>!!C</span>``」というスニペットが得られます。  
